...
@section('content')
    ...
    @if(isset($currentPackage) && $currentPackage)
        <div class="bg-blue-100 border border-blue-400 text-blue-800 px-4 py-3 rounded mb-4 text-center">
            You are currently subscribed to the <strong>{{ $currentPackage->name }}</strong> package.
        </div>
    @endif
    ...
    <div class="py-16 bg-gray-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 gap-8 md:grid-cols-{{ count($packages) }}">
                @foreach($packages as $package)
                <div class="bg-white rounded-lg shadow-lg overflow-hidden {{ $loop->index === 1 ? 'transform scale-105' : '' }}">
                    @if($loop->index === 1)
                    ...
                    <div class="px-6 py-8">
                        ...
                        @if(isset($activeSubscription) && $activeSubscription && $activeSubscription->subscription_package_id == $package->id)
                        ...
                        <div class="mt-6">
                            <span class="text-4xl font-extrabold text-gray-900">UGX {{ number_format($package->price) }}</span>
                            <span class="text-gray-500">/{{ $package->subscription_type }}</span>
                        </div>
                        <ul class="mt-8 space-y-4">
                            ...
                            <li class="flex items-center">
                                ...
                                <span class="ml-3 text-gray-700">{{ $package->duration_days }} days access</span>
                            </li>
                            <li class="flex items-center">
                                <svg class="h-5 w-5 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                </svg>
                                <span class="ml-3 text-gray-700">{{ $package->maximum_active_sessions }} Active Sessions</span>
                            </li>
                            @if($package->access_to_notices)
                            <li class="flex items-center">
                                <svg class="h-5 w-5 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                </svg>
                        ...
                        @if($package->downloadable_content || $package->practice_questions_bank || $package->performance_analytics || 
                        ...
                        <div class="mt-6 pt-6 border-t border-gray-200">
                            ...
                            <ul class="mt-4 space-y-4">
                                @if($package->downloadable_content)
                                <li class="flex items-center">
                                    <svg class="h-5 w-5 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                    </svg>
                                ...
                                @endif
                                @if($package->practice_questions_bank)
                                <li class="flex items-center">
                                    <svg class="h-5 w-5 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                    </svg>
                                ...
                                @endif
                                @if($package->performance_analytics)
                                <li class="flex items-center">
                                    <svg class="h-5 w-5 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                    </svg>
                                ...
                                @endif
                                @if($package->parent_progress_reports)
                                <li class="flex items-center">
                                    <svg class="h-5 w-5 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                    </svg>
                                ...
                                @endif
                                @if($package->one_on_one_tutoring_sessions)
                                <li class="flex items-center">
                                    <svg class="h-5 w-5 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                    </svg>
                        ...
                        <div class="mt-8">
                            @if(Auth::check())
                                ...
                                @endphp
                                @if($showEasypay || $showFlutterwave)
                                    ...
                                    @else
                                        @if($showEasypay)
                                            <form method="POST" action="{{ route('payment.easypay') }}" style="margin-bottom: 0.5rem;">
                                                @csrf
                                                <input type="hidden" name="package_id" value="{{ $package->id }}">
                                                <input type="hidden" name="amount" value="{{ $package->price }}">
                                                <input type="hidden" name="currency" value="UGX">
                                                <input type="hidden" name="reference" value="{{ Auth::id() }}">
                                                <input type="hidden" name="reason" value="Payment for subscription">
                                                <input type="hidden" name="client_id" value="{{ $easypay->client_id }}">
                                                <input type="hidden" name="secret" value="{{ $easypay->secret }}">
                                                <input type="hidden" name="action" value="mmdeposit">
                                                <input type="hidden" name="username" value="{{ $easypay->client_id }}">
                                                <input type="hidden" name="password" value="{{ $easypay->secret }}">
                                                <div class="mb-2">
                                                    <label for="phone_{{ $package->id }}" class="block text-sm font-medium text-gray-700">Mobile Money Phone Number</label>
                                                    <input type="text" name="phone" id="phone_{{ $package->id }}" value="{{ Auth::user()->phone_number }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                                </div>
                                        ...
                                        @if($showFlutterwave)
                                            <form method="POST" action="{{ route('payment.flutterwave') }}">
                                                ...
                                                <input type="hidden" name="package_id" value="{{ $package->id }}">
                                                <input type="hidden" name="amount" value="{{ $package->price }}">
                                                <input type="hidden" name="currency" value="{{ $flutterwave->currency_code ?? 'UGX' }}">
                                                <input type="hidden" name="reference" value="{{ Auth::id() }}">
                                                <input type="hidden" name="reason" value="Payment for subscription">
                                ...
                                @endif
                            @else
                            <a href="{{ route('register') }}" class="block w-full bg-blue-600 text-white text-center px-6 py-3 rounded-md font-semibold hover:bg-blue-700">
                                Get Started
                            </a>
                            @endif
                        </div>
        ...
        <div class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:py-16 lg:px-8 lg:flex lg:items-center lg:justify-between">
            ...
            <div class="mt-8 flex lg:mt-0 lg:flex-shrink-0">
                ...
                <div class="ml-3 inline-flex rounded-md shadow">
                    <a href="{{ route('contact') }}" class="inline-flex items-center justify-center px-5 py-3 border border-transparent text-base font-medium rounded-md text-white bg-blue-500 hover:bg-blue-400">
                        Contact us
                    </a>
                </div>
            </div>
