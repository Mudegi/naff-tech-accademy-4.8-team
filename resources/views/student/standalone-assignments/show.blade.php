...
<div class="assignment-detail-page">
    <div class="page-header">
        <a href="{{ route('student.assignments.index') }}" class="back-button">
            <i class="fas fa-arrow-left"></i>
            <span>Back to My Assignments</span>
        </a>
        <h1><i class="fas fa-clipboard-list"></i> {{ $assignment->title }}</h1>
        <div class="assignment-meta">
            <span class="badge badge-primary"><i class="fas fa-book"></i> {{ $assignment->subject->name ?? 'N/A' }}</span>
            <span class="badge badge-secondary"><i class="fas fa-users"></i> {{ $assignment->classRoom->name ?? 'N/A' }}</span>
            @if($assignment->due_date)
                <span class="badge {{ $assignment->isOverdue() ? 'badge-danger' : 'badge-info' }}">
                    <i class="fas fa-calendar"></i> Due: {{ $assignment->due_date->format('M d, Y') }}
                </span>
            @endif
            <span class="badge badge-success"><i class="fas fa-star"></i> {{ $assignment->total_marks }} marks</span>
        </div>
    </div>
            ...
            <div class="content-section">
                ...
                <div class="file-card">
                    ...
                    <div class="file-details">
                        <p><strong>{{ basename($assignment->assignment_file_path) }}</strong></p>
                        <p class="text-muted">{{ strtoupper($assignment->assignment_file_type ?? 'FILE') }}</p>
                    </div>
                    <a href="{{ route('student.assignments.download', $assignment->id) }}" class="btn btn-primary">
                ...
                <div class="submission-card submitted">
                    ...
                    @if($submission->student_comment)
                        <div class="submission-detail">
                            ...
                            <p>{{ $submission->student_comment }}</p>
                        </div>
                    @endif
                    ...
                    @if($submission->grade !== null)
                        <div class="grade-display">
                            ...
                            <span class="grade-value">{{ $submission->grade }}/{{ $assignment->total_marks }}</span>
                            <span class="grade-percentage">({{ round(($submission->grade / $assignment->total_marks) * 100, 1) }}%)</span>
                        </div>
                    @endif
                    ...
                    <div class="submission-actions">
                        <a href="{{ route('student.assignment-submissions.download', $submission->id) }}" class="btn btn-secondary">
...
document.getElementById('submissionForm').addEventListener('submit', function(e) {
    ...
    xhr.open('POST', '{{ route('student.assignments.submit', $assignment->id) }}');
    xhr.send(formData);
