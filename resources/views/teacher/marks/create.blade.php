...
<div class="dashboard-page">
    ...
    <div class="dashboard-card" id="bulkUploadForm">
        ...
        <form action="{{ route('teacher.marks.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <select name="class_id" id="class_id" required style="width: 100%; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 1rem;" onchange="loadClassStudents(this.value)">
                    ...
                    @foreach($classes as $c)
                        <option value="{{ $c->id }}" {{ (old('class_id') == $c->id || ($class && $class->id == $c->id)) ? 'selected' : '' }}>
                            {{ $c->name }} ({{ $c->grade_level ?? 'N/A' }})
                        </option>
                    @endforeach
                </select>
                @error('class_id')
            ...
            @if($class && $students->count() > 0)
                <div style="margin-bottom: 1.5rem; padding: 1rem; background: #f0f9ff; border-radius: 0.5rem; border: 1px solid #bae6fd;">
                    <h3 style="margin: 0 0 0.5rem 0; color: #1e40af;">Students in {{ $class->name }}</h3>
                    <p style="margin: 0; color: #1e3a8a; font-size: 0.875rem;">
                        Found {{ $students->count() }} student(s). Make sure your file uses student names or registration numbers that match these students.
                    </p>
                    <details style="margin-top: 1rem;">
                        ...
                        <ul style="margin-top: 0.5rem; padding-left: 20px; color: #1e3a8a;">
                            @foreach($students as $student)
                                <li>{{ $student->name }} @if($student->student && $student->student->registration_number)({{ $student->student->registration_number }})@endif</li>
                            @endforeach
                        </ul>
                    </details>
                </div>
            @elseif($class && $students->count() == 0)
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <select name="academic_level" id="academic_level" required style="width: 100%; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 1rem;">
                    ...
                    @foreach($academicLevels as $key => $label)
                        <option value="{{ $key }}" {{ old('academic_level') == $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                @if(!empty($teachingSubjects))
                    <select name="subject_name" id="subject_name" required style="width: 100%; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 1rem;">
                        ...
                        @foreach($teachingSubjects as $subject)
                            <option value="{{ $subject }}" {{ old('subject_name') == $subject ? 'selected' : '' }}>{{ $subject }}</option>
                        @endforeach
                    </select>
                ...
                @else
                    <input type="text" name="subject_name" id="subject_name" value="{{ old('subject_name') }}" required style="width: 100%; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 1rem;" placeholder="e.g., Mathematics, Physics, Chemistry">
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <input type="text" name="paper_name" id="paper_name" value="{{ old('paper_name') }}" style="width: 100%; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 1rem;" placeholder="e.g., Paper 1, Paper 2">
                @error('paper_name')
                    <span style="color: #dc2626; font-size: 0.875rem; margin-top: 0.25rem; display: block;">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <input type="number" name="academic_year" id="academic_year" value="{{ old('academic_year', date('Y')) }}" min="2000" max="{{ date('Y') + 1 }}" style="width: 100%; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 1rem;">
                @error('academic_year')
                    <span style="color: #dc2626; font-size: 0.875rem; margin-top: 0.25rem; display: block;">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div style="margin-bottom: 1.5rem;">                <label for="exam_type_single" style="display: block; margin-bottom: 0.5rem; font-weight: 600; color: #374151;">Exam Type *</label>
                <select name="exam_type" id="exam_type_single" required style="width: 100%; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 1rem;" onchange="toggleOtherExamTypeSingle(this)">
                    ...
                    <option value="Beginning of Term" {{ old('exam_type') == 'Beginning of Term' ? 'selected' : '' }}>Beginning of Term Exams</option>
                    <option value="Mid Term" {{ old('exam_type') == 'Mid Term' ? 'selected' : '' }}>Mid Term Exams</option>
                    <option value="End of Term" {{ old('exam_type') == 'End of Term' ? 'selected' : '' }}>End of Term Exams</option>
                    <option value="Mock" {{ old('exam_type') == 'Mock' ? 'selected' : '' }}>Mock Exams</option>
                    <option value="Other" {{ old('exam_type') == 'Other' ? 'selected' : '' }}>Other (Specify)</option>
                </select>
            ...
            <div id="exam_type_other_field_single" style="margin-bottom: 1.5rem; display: {{ old('exam_type') == 'Other' ? 'block' : 'none' }};">
                ...
                <input type="text" name="exam_type_other" id="exam_type_other_single" value="{{ old('exam_type_other') }}" style="width: 100%; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 1rem;" placeholder="e.g., Pre-Mock, Weekly Test">
            ...
            <div id="exam_type_other_field" style="margin-bottom: 1.5rem; display: {{ old('exam_type') == 'Other' ? 'block' : 'none' }};">
                <label for="exam_type_other" style="display: block; margin-bottom: 0.5rem; font-weight: 600; color: #374151;">Specify Exam Type</label>
                <input type="text" name="exam_type_other" id="exam_type_other" value="{{ old('exam_type_other') }}" style="width: 100%; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 1rem;" placeholder="e.g., Pre-Mock, Weekly Test">
                @error('exam_type_other')
                    <span style="color: #dc2626; font-size: 0.875rem; margin-top: 0.25rem; display: block;">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <p style="margin-top: 0.5rem; color: #6b7280; font-size: 0.875rem;">
                    <a href="{{ route('teacher.marks.template', ['format' => 'csv']) }}" style="color: #2563eb; text-decoration: underline;">Download CSV Template</a> | 
                    <a href="{{ route('teacher.marks.template', ['format' => 'excel']) }}" style="color: #2563eb; text-decoration: underline;">Download Excel Template</a>
                </p>
                @error('marks_file')
                    <span style="color: #dc2626; font-size: 0.875rem; margin-top: 0.25rem; display: block;">{{ $message }}</span>
                @enderror
        ...
        <form action="{{ route('teacher.marks.store') }}" method="POST">
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <select name="class_id" id="single_class_id" required style="width: 100%; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 1rem;" onchange="loadStudentsForSingle(this.value)">
                    <option value="">-- Select a Class --</option>
                    @foreach($classes as $c)
                        <option value="{{ $c->id }}">{{ $c->name }} ({{ $c->grade_level ?? 'N/A' }})</option>
                    @endforeach
                </select>
            </div>
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <input type="number" name="academic_year" id="single_academic_year" value="{{ date('Y') }}" min="2000" max="{{ date('Y') + 1 }}" style="width: 100%; padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 1rem;">
            </div>
...
function loadClassStudents(classId) {
    if (classId) {
        window.location.href = '{{ route("teacher.marks.create") }}?class_id=' + classId;
    }
}
