...
<div class="dashboard-content-inner">
    <div class="dashboard-breadcrumbs">
        ...
        <a href="{{ route('admin.resources.index') }}" class="dashboard-btn dashboard-btn-secondary">
    ...
    <div class="dashboard-card">
        <form action="{{ route('admin.resources.update', $resource->hash_id) }}" method="POST" enctype="multipart/form-data" class="resource-form">
            @csrf
            @method('PUT')
            ...
            <div class="form-grid">
                ...
                <div class="form-section">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $resource->title) }}" class="form-input" required>
                    </div>

                    <div class="form-group">
                        <label for="description" class="form-label">Description</label>
                        <textarea name="description" id="description" rows="3" class="form-textarea">{{ old('description', $resource->description) }}</textarea>
                    </div>
                ...
                <div class="form-section">
                    ...
                    <div class="form-group">
                        ...
                        <select name="class_id" id="class_id" class="form-select">
                            ...
                            @foreach($classes as $class)
                                <option value="{{ $class->id }}" {{ old('class_id', $resource->class_id) == $class->id ? 'selected' : '' }}>{{ $class->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="topic_id" id="topic_id" class="form-select" required>
                            ...
                            @foreach($topics as $topic)
                                <option value="{{ $topic->id }}" {{ old('topic_id', $resource->topic_id) == $topic->id ? 'selected' : '' }}>{{ $topic->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="term_id" id="term_id" class="form-select" required>
                            ...
                            @foreach($terms as $term)
                                <option value="{{ $term->id }}" {{ old('term_id', $resource->term_id) == $term->id ? 'selected' : '' }}>{{ $term->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="teacher_id" id="teacher_id" class="form-select">
                            ...
                            @foreach($teachers as $teacher)
                                <option value="{{ $teacher->id }}" {{ old('teacher_id', $resource->teacher_id) == $teacher->id ? 'selected' : '' }}>{{ $teacher->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="alert alert-info">
                        ...
                        @if($resource->school_id)
                            <strong>{{ $resource->school->name ?? 'Unknown' }}</strong> (Primary)
                        @endif
                        @if($resource->schools->count() > 0)
                            @if($resource->school_id), @endif
                            <strong>{{ $resource->schools->pluck('name')->join(', ') }}</strong> (Additional)
                        @endif
                        @if(!$resource->school_id && $resource->schools->count() == 0)
                    ...
                    <div class="form-group">
                        ...
                        <select name="school_id" id="school_id" class="form-select">
                            ...
                            @foreach($schools as $school)
                                <option value="{{ $school->id }}" {{ old('school_id', $resource->school_id) == $school->id ? 'selected' : '' }}>
                                    {{ $school->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select name="school_ids[]" id="school_ids" class="form-select" multiple size="5" style="height: auto; min-height: 120px;">
                            @foreach($schools as $school)
                                <option value="{{ $school->id }}" {{ in_array($school->id, old('school_ids', $resource->schools->pluck('id')->toArray())) ? 'selected' : '' }}>
                                    {{ $school->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <input type="url" name="video_url" id="video_url" value="{{ old('video_url', $resource->video_url) }}" class="form-input" placeholder="https://youtube.com/...">
                    ...
                    @if($resource->video_url)
                        <a href="{{ route('admin.resources.video-view', $resource->hash_id) }}" target="_blank" class="dashboard-btn dashboard-btn-primary" style="margin-top:10px;display:inline-block;">View Larger</a>
                    @endif
                </div>

                <div class="form-group">
                    <label for="google_drive_link" class="form-label">Google Drive Link</label>
                    <input type="url" name="google_drive_link" id="google_drive_link" value="{{ old('google_drive_link', $resource->google_drive_link) }}" class="form-input" placeholder="https://drive.google.com/file/d/...">
                    ...
                    @if($resource->google_drive_link)
                        <a href="{{ route('admin.resources.drive-play', $resource->hash_id) }}" target="_blank" class="dashboard-btn dashboard-btn-primary" style="margin-top:10px;display:inline-block;">View Larger</a>
                    @endif
                </div>
                ...
                <div class="form-group">
                    ...
                    @if($resource->notes_file_path)
                        <div class="current-file">
                            <span class="file-name">{{ basename($resource->notes_file_path) }}</span>
                            <a href="{{ asset('storage/' . $resource->notes_file_path) }}" target="_blank" class="file-download">
                ...
                <div class="form-group">
                    ...
                    @if($resource->assessment_tests_path)
                        <div class="current-file">
                            <span class="file-name">{{ basename($resource->assessment_tests_path) }}</span>
                            <a href="{{ asset('storage/' . $resource->assessment_tests_path) }}" target="_blank" class="file-download">
                                <i class="fas fa-download"></i> Download
                            </a>
                        </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="tags" id="tags" value="{{ old('tags', $resource->tags) }}" class="form-input" placeholder="tag1, tag2, tag3">
                </div>
                ...
                <div class="form-group">
                    ...
                    @endphp
                    <textarea name="learning_outcomes" id="learning_outcomes" class="form-textarea" rows="4" placeholder="Enter learning outcomes separated by commas. Example: Understand basic algebra, Solve quadratic equations, Apply mathematical concepts">{{ $learningOutcomesText }}</textarea>
                ...
                <div class="form-group">
                    <label class="form-checkbox">
                        <input type="checkbox" name="visible_as_sample" value="1" {{ old('visible_as_sample', $resource->visible_as_sample) ? 'checked' : '' }}>
