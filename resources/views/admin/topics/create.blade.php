...
<div class="dashboard-content-inner">
    <div class="dashboard-breadcrumbs" style="display: flex; justify-content: space-between; align-items: center;">
        <h1 class="dashboard-title">Add Topic</h1>
    ...
    <div class="profile-card" style="max-width:600px;margin:0 auto;">
        <form method="POST" action="{{ route('admin.topics.store') }}" id="topicForm">
            @csrf
            ...
            <div class="profile-form-group">
                ...
                <input type="number" id="order" name="order" value="{{ old('order', 0) }}" class="profile-input">
            ...
            <button type="submit" class="dashboard-btn dashboard-btn-primary">Create Topic</button>
        </form>
...
topicForm.addEventListener('submit', function(e) {
    ...
    .then(data => {
        if (data.success) {
            showSuccess(data.message || 'Topic created successfully!');
            setTimeout(() => {
            ...
            if (data.errors) {
                ...
                showError(data.message || 'An error occurred while creating the topic.');
            }
    ...
    .catch(error => {
        showError(error.message || 'An error occurred while creating the topic. Please try again.');
        submitButton.disabled = false;
        submitButton.textContent = 'Create Topic';
    });
});
