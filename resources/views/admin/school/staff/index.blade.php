...
<div class="dashboard-content-inner staff-page">
    ...
    <div class="dashboard-breadcrumbs">
        ...
        <a href="{{ route('admin.school.staff.create') }}" class="btn-modern btn-primary">
    ...
    @if (session('show_credentials') && session('new_staff_credentials'))
        @php
                    ...
                    <div class="credentials-info-section">
                        ...
                        <div class="credentials-info-list">
                            <div class="credentials-info-item">
                                ...
                                <span class="credentials-info-value">{{ $credentials['name'] }}</span>
                            </div>
                            <div class="credentials-info-item">
                                <span class="credentials-info-label">Email:</span>
                                <span class="credentials-info-value">{{ $credentials['email'] }}</span>
                            </div>
                            @if($credentials['phone_number'])
                            <div class="credentials-info-item">
                                <span class="credentials-info-label">Phone:</span>
                                <span class="credentials-info-value">{{ $credentials['phone_number'] }}</span>
                            </div>
                            @endif
                            <div class="credentials-info-item">
                                <span class="credentials-info-label">Role:</span>
                                <span class="credentials-info-value credentials-role-badge">{{ ucfirst(str_replace('_', ' ', $credentials['account_type'])) }}</span>
                            </div>
                        </div>
                    </div>
                    <div class="credentials-login-section">
                        ...
                        <div class="credentials-login-list">
                            <div class="credentials-login-item">
                                ...
                                <code class="credentials-code" id="new-email-display">{{ $credentials['email'] }}</code>
                            </div>
                            <div class="credentials-login-item">
                                <span class="credentials-login-label">Password:</span>
                                <code class="credentials-code" id="password-display">{{ $credentials['password'] }}</code>
                            </div>
    ...
    @if (session('show_updated_credentials') && session('updated_staff_credentials'))
        @php
                ...
                <div class="credentials-info-grid">
                    ...
                    <div class="credentials-login-section">
                        ...
                        <div class="credentials-login-list">
                            <div class="credentials-login-item">
                                <span class="credentials-login-label">Email:</span>
                                <code class="credentials-code" id="updated-email-display">{{ $credentials['email'] }}</code>
                            </div>
                            <div class="credentials-login-item">
                                <span class="credentials-login-label">New Password:</span>
                                <code class="credentials-code" id="updated-password-display">{{ $credentials['password'] }}</code>
                            </div>
    ...
    <div class="search-filter-card">
        <form method="GET" action="{{ route('admin.school.staff.index') }}" class="search-filter-form">
            <div class="search-input-group">
                ...
                <select name="account_type" class="filter-select">
                    ...
                    <option value="director_of_studies" {{ request('account_type') == 'director_of_studies' ? 'selected' : '' }}>Director of Studies</option>
                    <option value="head_of_department" {{ request('account_type') == 'head_of_department' ? 'selected' : '' }}>Head of Department</option>
                    <option value="subject_teacher" {{ request('account_type') == 'subject_teacher' ? 'selected' : '' }}>Subject Teacher</option>
                </select>
    ...
    <div class="table-card-modern">
        <div class="table-header-modern">
            <div class="table-header-left">
                ...
                <span class="table-count-badge">{{ $staff->total() }} {{ Str::plural('Member', $staff->total()) }}</span>
            </div>
        </div>
                    ...
                    <tr class="table-row-modern">
                        ...
                        <td>
                            ...
                            <span class="role-badge {{ $roleClass }}">
                                ...
                                {{ ucfirst(str_replace('_', ' ', $member->account_type)) }}
                            </span>
                        </td>
                        <td>
                            @if($member->department)
                                <a href="{{ route('admin.school.departments.show', $member->department->id) }}" class="department-link-badge">
                        ...
                        <td>
                            <span class="status-badge-modern {{ $member->is_active ? 'status-active-modern' : 'status-inactive-modern' }}">
                        ...
                        <td>
                            <div class="action-buttons">
                                <a href="{{ route('admin.school.staff.edit', $member->id) }}" class="action-btn action-edit" title="Edit">
                                    <i class="fas fa-edit"></i>
                                </a>
                                @if(in_array($member->account_type, ['teacher', 'subject_teacher']) && (Auth::user()->isSchoolAdmin() || Auth::user()->isDirectorOfStudies()))
                                <a href="{{ route('admin.school.staff.assign-classes', $member->id) }}" class="action-btn action-assign" title="Assign Classes" style="background: #10b981; color: white;">
                                    <i class="fas fa-chalkboard-teacher"></i>
                                </a>
                                @endif
                                @if(Auth::user()->canManageUser($member) && $member->id !== Auth::id())
                                <form action="{{ route('admin.school.staff.destroy', $member->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this staff member?');">
                                    @csrf
                                    @method('DELETE')
...
function copyCredentials() {
    const email = document.getElementById('new-email-display')?.textContent || '{{ session("new_staff_credentials.email") ?? "" }}';
    const password = document.getElementById('password-display')?.textContent || '{{ session("new_staff_credentials.password") ?? "" }}';
...
function copyUpdatedCredentials() {
    const email = document.getElementById('updated-email-display')?.textContent || '{{ session("updated_staff_credentials.email") ?? "" }}';
    const password = document.getElementById('updated-password-display')?.textContent || '{{ session("updated_staff_credentials.password") ?? "" }}';
